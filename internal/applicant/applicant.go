...
func applyLetsencrypt(option *ApplyOption, provider challenge.Provider) (*Certificate, error) {
...
func applyZeroSSL(option *ApplyOption, provider challenge.Provider) (*Certificate, error) {
	privateKey, err := ecdsa.GenerateKey(elliptic.P256(), rand.Reader)
	if err != nil {
		return nil, err
	}
	...
	eabOptions := registration.RegisterEABOptions{
		...
		Kid:                  zerosslKid,
		HmacEncoded:          zerosslHMAC,
	}

	// 注册用户并处理返回的 Registration 资源和错误
